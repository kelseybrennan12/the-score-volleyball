...
| Season     | [/src/shared/domain/seasons.ts](/src/shared/domain/seasons.ts)       | A `(session, year)` pair, keyed `<session>-<year>` (e.g. `spring-2026`). The live season is in `active/`; retired seasons are frozen under `seasons/`. |
...
| Snapshot repo      | [/src/backend/runtime/adapters/snapshots/fs.ts](/src/backend/runtime/adapters/snapshots/fs.ts)                             | Filesystem reader/writer for `data/snapshots/active/`, `archive/`, and `seasons/`.                         |
...
| Season archive CLI | [/src/backend/season-archive.entry.ts](/src/backend/season-archive.entry.ts)                                               | `mise run archive-season` entrypoint; freezes a retired season into `seasons/` and purges its live copies. |
...
| Previous Seasons | [/src/components/standings-view.tsx](/src/components/standings-view.tsx)       | Collapsible section under current standings; browses frozen `seasons/` archives with the reused pill row + table. Selection is local (non-URL) state. |
